...
export function gettingZoomLevel(distance: number = 18489298) {
  const data = {
    19: 1128.49722,
    18: 2256.99444,
    17: 4513.98888,
  ...
  for (const key in data) {
    if (data.hasOwnProperty(key) && data[key] <= distance) {
      matchingKey = parseInt(key) - 1;
      break; // Stop as soon as you find the first matching key
    }
  }
  return matchingKey;
}
